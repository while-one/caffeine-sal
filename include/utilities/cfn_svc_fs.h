...
typedef struct
{
    void *instance; /*!< Mapping to SD Card, Flash, or Internal NVM driver */
    void *user_arg;
} cfn_svc_fs_phy_t;
...
CFN_SVC_CREATE_DRIVER_TYPE(svc_fs, cfn_svc_fs_config_t, cfn_svc_fs_api_t, cfn_svc_fs_phy_t, cfn_svc_fs_callback_t);

#define CFN_SVC_FS_INITIALIZER(api_ptr, phy_ptr, config_ptr)                                                           \
    CFN_SVC_DRIVER_INITIALIZER(CFN_SVC_TYPE_FS, api_ptr, phy_ptr, config_ptr)
